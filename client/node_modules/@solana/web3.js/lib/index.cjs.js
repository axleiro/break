...
function createRpcResult(result) {
  return superstruct.union([superstruct.type({
    ...
    error: superstruct.type({
      ...
      data: superstruct.optional(superstruct.unknown())
    })
  })]);
}
...
const GetLeaderScheduleResult = superstruct.record(superstruct.string(), superstruct.unknown());
/**
...
const SignatureProcessedResult = superstruct.type({
  type: superstruct.literal('processedSignature'),
...
const SignatureReceivedResult = superstruct.type({
  type: superstruct.literal('receivedSignature'),
  timestamp: superstruct.number()
});
/**
 * Signature status for a transaction
...
const SignatureSubscribedResult = superstruct.type({
  type: superstruct.literal('subscribedSignature'),
  timestamp: superstruct.number()
});
/**
...
const Version = superstruct.type({
  'solana-core': superstruct.string(),
  'feature-set': superstruct.optional(superstruct.nullable(superstruct.number()))
});
...
const SlotUpdateResult = superstruct.union([superstruct.type({
  ...
  slot: superstruct.number(),
  timestamp: superstruct.number(),
  entry_stats: superstruct.type({
    numTransactions: superstruct.number(),
    numEntries: superstruct.number(),
    maxTxPerEntry: superstruct.number()
  })
}), superstruct.object({
...
const SignatureNotificationResult = superstruct.type({
  subscription: superstruct.number(),
  result: notificationResultAndContext(superstruct.union([SignatureProcessedResult, SignatureReceivedResult, SignatureSubscribedResult]))
});
/**
...
const SignatureStatusResponse = superstruct.type({
  ...
  err: TransactionErrorResult,
  confirmationStatus: superstruct.optional(superstruct.nullable(superstruct.string()))
});
/**
...
class Connection {
  ...
  async getVersion() {
    ...
    const res = superstruct.create(unsafeRes, jsonRpcResult(Version));
  ...
  async validatorExit() {
    ...
    const res = superstruct.create(unsafeRes, jsonRpcResult(superstruct.boolean()));
  ...
  _wsOnSlotUpdatesNotification(notification) {
    ...
    invariant__default['default'](typeof res.result !== 'undefined');
...
class Secp256k1Program {
  ...
  static createInstructionWithPublicKey(params) {
    ...
    invariant__default['default'](publicKey.length === PUBLIC_KEY_BYTES, `Public key must be ${PUBLIC_KEY_BYTES} bytes`);
    let ethPublicKey;
  ...
  static createInstructionWithPrivateKey(params) {
    ...
    invariant__default['default'](privateKey.length === PRIVATE_KEY_BYTES, `Private key must be ${PRIVATE_KEY_BYTES} bytes`);
